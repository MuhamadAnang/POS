    ...
    <div class="card card-outline card-primary">
        ...
        <div class="card-body">
            <form method="POST" action="{{ url('/transaksi/' . $transaksi->penjualan_id) }}" class="form-horizontal">
                    ...
                    <div class="form-group col">
                        ...
                        <div class="col">
                            <input type="text" class="form-control" id="pembeli" name="pembeli"
                                value="{{ $transaksi->pembeli ?? old('pembeli') }}" required>
                            @error('pembeli')
                        ...
                        <div class="col">
                            <select class="form-control " id="" name="user_id" required>
                                ...
                                @foreach ($listUser as $user)
                                    <option value="{{ $user->user_id }}" @if ($user->user_id == $transaksi->user_id) selected @endif>
                                        {{ $user->nama }}</option>
                                @endforeach
                            </select>
                            @error('pembeli')
                    ...
                    <div class="form-group col-sm-6">
                        ...
                        <div class="col">
                            <input type="text" class="form-control" id="penjualan_kode" name="penjualan_kode"
                                value="{{ $transaksi->penjualan_kode }}" readonly>
                    ...
                    <div class="form-group col-sm-6">
                        ...
                        <div class="col">
                            <input type="date" class="form-control" id="penjualan_tanggal" name="penjualan_tanggal"
                                value="{{ date('Y-m-d', strtotime($transaksi->penjualan_tanggal)) ?? date('Y-m-d') }}"
                                required>
                ...
                <div class="row">
                    <div class="form-group col">
                        <label class="col control-label col-form-label">Diedit Oleh: </label>
                        <div class="col">
                            <select class="form-control " id="" name="updated_by" required>
                                <option value="">Pilih User</option>
                                @foreach ($listUser as $user)
                                    <option value="{{ $user->user_id }}" @if ($user->user_id == $transaksi->user_id) selected @endif>
                                        {{ $user->nama }}</option>
                                        ...
                                        <select class="form-control barang" id="" name="barang_id[]" required>
                                            ...
                                            @foreach ($listBarang as $barang)
                                                <option value="{{ $barang->barang_id }}"
                                                    @if ($barang->barang_id == $detail->barang_id) selected @endif>
                                                    {{ $barang->barang_nama }}
                                                </option>
                                            @endforeach
                                        </select>
                                ...
                                <div class="form-group col-sm-6">
                                    ...
                                    <div class="col">
                                        <input type="text" class="form-control jumlah" id="jumlah" name="jumlah[]"
                                            ...
                                            data-old="{{ $detail->jumlah }}" required placeholder="Min. 1">
                                ...
                                <div class="form-group col-sm-6">
                                    ...
                                    <div class="col">
                                        <input type="text" class="form-control harga" id="harga"
                                            value="{{ old('total') ?? $detail->barang->harga_jual }}" readonly>
                                        @error('harga')
                                            <small class="form-text text-danger">{{ $message }}</small>
                                        @enderror
                                    </div>
                                ...
                                <div class="form-group col-sm-6">
                                    ...
                                    <div class="col">
                                        ...
                                        <input type="text" class="form-control total"
                                            data-old-total="{{ $detail->total }}" id="total" name="harga[]"
...
@push('js')
    <script>
        let barang = {};
        let selectedId = [...@json($transaksi->detail->pluck('barang_id'))];
        let oldTransaksi = @json($transaksi);
        let oldJumlah = 0;
        ...
        const fetchData = async (id) => {
            try {
                let response = await $.get(`/api/barang/${id}/get`, function(response) {
